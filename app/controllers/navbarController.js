'use strict';

define(['app'], function (app) {

    var injectParams = ['$scope', '$location', 'config', 'authService', '$state', '$localStorage'];

    var NavbarController = function ($scope, $location, config, authService, $state, $localStorage) {
        var vm = this,
            appTitle = 'Customer Management';

        vm.isCollapsed = false;
        vm.appTitle = (config.useBreeze) ? appTitle + ' Breeze' : appTitle;

        vm.highlight = function (path) {
            return $location.path().substr(0, path.length) === path;
        };

        vm.loginOrOut = function () {
            console.log('User logout ----------------');
            setLoginLogoutText();
            var isAuthenticated = $localStorage.user.isAuthenticated;
            if (isAuthenticated) { //logout 
                $localStorage.$reset(); //Delete Everything
//                console.log($localStorage.user);
//                authService.logout().then(function () {
//                    $location.path('/');
//                    return;
//                });
            }
            redirectToLogin();
        };

        function redirectToLogin() {
            
//            var path = '/login' + $location.$$path;
//            console.log(path);
//            $location.replace();
//            $location.path(path);
//            window.location = '#/login';
            $state.go('Login');
        }

        $scope.$on('loginStatusChanged', function (loggedIn) {
            setLoginLogoutText(loggedIn);
        });

        $scope.$on('redirectToLogin', function () {
            console.log('redirectToLogin');
            redirectToLogin();
        });

        function setLoginLogoutText() {
            vm.loginLogoutText = ($localStorage.user.isAuthenticated) ? 'Logout' : 'Login';
        }

        setLoginLogoutText();

    };

    NavbarController.$inject = injectParams;


    //Loaded normally since the script is loaded upfront 
    //Dynamically loaded controller use app.register.controller
    app.controller('NavbarController', NavbarController);

});